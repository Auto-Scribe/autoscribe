...
from .difficulty_adjuster import DifficultyAdjuster, DifficultyLevel
...
class MusicXMLExporter:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            difficulty_level: Target difficulty level (None = no adjustment)
        """
    ...
    def _preprocess(self, piano_roll: PianoRoll) -> PianoRoll:
        """
        Preprocess piano roll (quantize, adjust difficulty, etc.).
        ...
        if self.difficulty_level:
            print(f"Adjusting difficulty to {self.difficulty_level.value}...")
            adjuster = DifficultyAdjuster()
            adjuster.config.target_level = self.difficulty_level
            piano_roll = adjuster.adjust_difficulty(piano_roll)
            print(f" Difficulty adjusted")
    ...
    def _create_score(self, piano_roll: PianoRoll, title: Optional[str]) -> stream.Score:
        """
    ...
    def _create_part(self, 
                     piano_roll: PianoRoll, 
                     part_name: str, 
                     part_clef: clef.Clef) -> stream.Part:
        """
        ...
        # Add time signature
        ts = meter.TimeSignature(f"{piano_roll.time_signature[0]}/{piano_roll.time_signature[1]}")
        part.append(ts)
    ...
    def _notes_to_music21(self, notes: list, tempo: float) -> Optional[note.GeneralNote]:
        """
    ...
    def export_to_pdf(self, 
        ...
        """
        Export to PDF using MuseScore (if installed).
        ...
        """
        # First create MusicXML
        musicxml_path = output_path.replace('.pdf', '.musicxml')
        self.export(piano_roll, musicxml_path, title)
        
        # Try to convert to PDF with MuseScore
        try:
            from music21 import environment
            env = environment.Environment()
            
            # This requires MuseScore to be installed
            score = stream.Score()
            score.read(musicxml_path)
            ...
            print(" To create PDFs, install MuseScore and configure music21")
            print(f"  You can manually open {musicxml_path} in MuseScore and export to PDF")
            return None


def export_to_musicxml(piano_roll: PianoRoll, 
                       output_path: str,
                       title: Optional[str] = None,
                       auto_quantize: bool = True) -> str:
    """
